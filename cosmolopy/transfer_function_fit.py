...
class TransferFunctionfit:
    ...
    def tf_fit_k_mpc(self,k):
        ...
        T_c_ln_beta_1 = np.log(np.e + 1.8*q)
        T_c_C_alpha = 14.2 / self.alpha_c + 386.0 / (1.0+69.9 * q**1.08)
        T_c_C_alpha_1 = 14.2 + 386.0 / (1.0+69.9 * q**1.08)
        
        T_c_tilde_1 = T_c_ln_beta / (T_c_ln_beta + T_c_C_alpha_1 * q**2)
        T_c_tilde = T_c_ln_beta / (T_c_ln_beta + T_c_C_alpha * q**2)
        T_c_f = 1.0 / (1.0+(xx/5.4)**4)
        T_c = T_c_f * T_c_tilde_1 + (1.0-T_c_f) * T_c_tilde
        
        s_tilde = self.sound_horizon / (1.0+(self.beta_node / xx)**3)**(1./3.)
        xx_tilde = k * s_tilde
        
        T_c_tilde_1 = T_c_ln_beta_1 / (T_c_ln_beta_1 + T_c_C_alpha_1 * q**2)
        T_b_1 = T_c_tilde_1/(1.0+(xx/5.2)**2)
        T_b_2 = (self.alpha_b / (1.0+(self.beta_b/xx)**3)) * np.exp(-1*(k / self.k_silk)**1.4)
